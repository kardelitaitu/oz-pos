...
## Money — TDD sweep (`MONEY-01..05` — CLOSED 2026-08-31, one OPEN residual)
...
- **LOYALTY-01 (OPEN)** — `db/loyalty.rs:777-778` computes `points = ((base as f64)/100 × earn_multiplier).round() as i64` with the multiplier stored as `REAL`/`DOUBLE PRECISION` (base = total_minor × points_per_unit). Verified by exhaustive scan (double vs exact-decimal, half-away): **multiplier 1.4 flips at 585 bases ≤ 2,000,000** — e.g. base=2250 (a $22.50 sale at points_per_unit=1): float 31.499999999999996 → **31 points where exact decimal gives 32**; always DOWNWARD for 1.4 (its f64 sits below the decimal). Multipliers 1.1/1.2/1.3/1.5/1.75/2.0: zero flips in the same range (1.5/1.75/2.0 are exact in binary; the others' error snaps back at the tie). The corruption happens at WRITE time — the UI sends a JS number (`api/loyalty.ts:9`), IPC carries f64, the column is REAL — so no local patch at the compute site can recover the owner's intent. The honest fix is fixed-point millionths (the repo's own `rate_millionths` precedent): migration + backfill + DTO + wire + tier-editor changes = a schema decision, deferred. Counterexample pinned here.
