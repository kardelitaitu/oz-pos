...
- **REP-03** — Date boundaries have no store-timezone contract or input validation — **OPEN, design recorded 2026-08-31** (deliberately not half-fixed: mixed local/UTC bucketing across queries would be worse than uniform UTC). Contract sketch: `store_profiles.timezone` (TEXT, default 'UTC') is stored but unused by reports — all bucketing is `DATE(s.created_at)` on UTC strings, so a UTC+7 store attributes 00:00–07:00 local sales to the previous day. Fix requires: (a) decide the stored format (offset `±HH:MM` is directly consumable by SQLite date modifiers; IANA names need a `chrono-tz` dep + per-date DST resolution done Rust-side); (b) thread a validated modifier through ALL date-bucketed queries at once (daily/weekly/monthly revenue incl. the REP-04 refund CTEs, heatmap, breakdowns, avg basket, customer split, discounts, sent-report scheduling); (c) UI date pickers must submit store-local dates; (d) input validation on the boundary strings themselves (currently unvalidated `BETWEEN ?1 AND ?2`).
