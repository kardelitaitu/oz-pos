...
- **LOYALTY-01 (OPEN)** — `db/loyalty.rs:778` computes `points = ((base as f64)/100 × earn_multiplier).round() as i64` with the multiplier stored as `REAL`/`DOUBLE PRECISION`: base=250, multiplier=1.4 → float 3.4999999999999996 → 3 points where exact decimal half-away gives 4. Same boundary class as MONEY-01 but the honest fix is a schema decision (store multipliers as fixed-point millionths), not a local patch — tiers are user-created (no seeds), so custom multipliers make it reachable. Counterexample pinned here; deferred.
