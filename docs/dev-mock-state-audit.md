...
1. **Login lockout** — `loginAttempts` resets on reload, so a reloaded page bypasses a lockout the real backend keeps enforcing. The backend is also richer: sliding-window, per-account **and** per-device and global limits with exponential backoff (mock is a flat count of 4).
2. **Shift history** — `mockShiftHistory` loses closed shifts on reload; the backend's `shifts` table keeps them. (The one-item seed regenerates each load, so the list never looks empty — which can mask the loss.)
...
1. **Persist login lockout** under `oz-dev-mock:login-attempts` — otherwise a reload defeats the lockout in dev. Optionally mirror the backend's sliding-window model instead of the flat threshold.
2. **Persist shift history** under `oz-dev-mock:shift-history`.
3. **(Stretch) Exercise held carts** by implementing real `hold_cart`/`list_held_carts` state instead of hardcoded `[]` — unblocks the hold/resume flow in the preview.
