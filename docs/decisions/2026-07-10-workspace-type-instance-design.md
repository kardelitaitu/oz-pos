...
2. **Session Context** ✅ (struct only; extractor deferred to ADR #7)
   ...
   - [x] **ADR #7 migration underway:** `list_products_scoped` ✅, `adjust_stock_scoped` ✅, `lookup_by_barcode_scoped` ✅, `lookup_product_by_sku_scoped` ✅, `create_product_scoped` ✅, `update_product_scoped` ✅, `delete_product_scoped` ✅ — all with frontend API wrappers. Remaining domain commands (orders, customers, etc.) deferred.
