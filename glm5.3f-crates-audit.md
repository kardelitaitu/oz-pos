...
| ID | Sev | Location | Finding | Proposed solution |
|---|---|---|---|---|
| LSE-11 | 🟡 MED — design tradeoff, needs a product decision | activate.go:482–507 (re-activation api_key rotation) | Anyone who knows a tenant's **email + full license key string** can force an api_key rotation and receive the new credential: the re-activation path accepts email+key as sufficient proof (deliberate, for reinstalls that lost the api_key), and when `verifyAPIKey` fails it rotates the tenant's api_key and returns the plaintext to the caller. Impact: an attacker holding both values (keys leak via screenshots/support/resale; emails via receipts) silently takes over the tenant's api-key-gated surface (`/renew`, `/status`) and invalidates the victim's real key. No financial minting — the signed subscription is untouched — but it is a silent credential takeover of a paying tenant. | Options: (a) require web-OTP inbox proof for rotation (the OTP store already exists — email the owner a code before rotating); (b) rate-limit + notify: at most one rotation per tenant per 24 h, owner notified by email on every rotation (turns silent takeover into a detected anomaly); (c) drop rotation entirely and make lost-key recovery a support flow. Implementing any of these changes a deliberate tradeoff, so it is recorded for the owner to choose. |
...
**Slice D remainder (next):** `admin_stats.go` (394), `trial.go` (256), `renew.go` (228), `status.go` (159), `pause.go`/`resume.go`/`expiry.go` (186), `contact.go` (176), `smtp_mail.go` (201), `health.go` (198), `enterprise_trial.go` (170), `trial_emails.go` remainder — then the build-artifact hygiene check.