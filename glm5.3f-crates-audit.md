...
| LSE-15 | 🟡 MED — billing correctness, recorded for a product decision | resume.go:51–54 | **Resume never extends `expires_at`** — the doc comment claims "resets the billing cycle from now" but the code only flips status and clears the pause fields. A customer who pauses 3 months and resumes loses those paid months. Worse: there is **no auto-resume scheduler anywhere** (`paused_until` is only honored lazily when a resume call arrives), so a "paused" subscription that nobody resumes just sits expired-looking while the device's signed payload (still `active`, original expiry) keeps working offline. | Proper fix: on resume, extend `expires_at` (+ `grace_until`) by the actual paused duration **and re-sign the payload** like renew does (otherwise /status and the device payload disagree). Requires a product call on pause semantics — recorded, not implemented. Also decide: background scanner for auto-resume, or drop the auto-resume claim from the docs. |
...
**Section 45 fix tally:** HIGH fixed: LSE-1, LSE-17, LSE-23. MED fixed: LSE-5, LSE-6, LSE-8, LSE-9, LSE-11 (A+B), LSE-13, LSE-14, LSE-16, LSE-18, LSE-21, LSE-22. LOW fixed: LSE-19, LSE-20, LSE-25 (+B43/B45 lineage fixes). MED recorded for a product decision: **LSE-15** (pause/resume billing semantics — needs extend+re-sign like renew; no auto-resume scheduler). INFO-only files: api_key.go, status.go, expiry.go, health.go, admin_stats.go, healthcheck.go, helpers.go.
