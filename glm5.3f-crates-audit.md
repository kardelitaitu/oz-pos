...
| CICD-05 | 🟢 LOW — recorded | all workflows | Third-party actions pinned by major/major.minor tag (`softprops/action-gh-release@v2`, `dorny/paths-filter@v3`, `aquasecurity/trivy-action@v0.36.0`, `docker/*@v3/v5`, `signpath/*@v2`, `actions/*`), not by commit SHA — the standard supply-chain hardening is SHA pinning for secret-touching jobs. | Recorded: pin to full SHAs in a follow-up pass (mechanical; the `release` environment approval gate mitigates the highest-value target until then). |
