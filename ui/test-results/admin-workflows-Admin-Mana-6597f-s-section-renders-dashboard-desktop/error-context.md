...
- Name: admin-workflows.spec.ts >> Admin Management Screens >> Stores section renders dashboard
- Location: e2e\admin-workflows.spec.ts:85:3
...
Locator: locator('.settings-nav-item').filter({ hasText: 'Stores' })
Expected: visible
Timeout: 3000ms
...
Call log:
  ...
  - waiting for locator('.settings-nav-item').filter({ hasText: 'Stores' })
