...
- Name: admin-workflows.spec.ts >> Admin Management Screens >> Tax Rates section renders with table
- Location: e2e\admin-workflows.spec.ts:73:3
...
Locator: locator('.settings-nav-item').filter({ hasText: 'Tax Rates' })
Expected: visible
Timeout: 3000ms
...
Call log:
  ...
  - waiting for locator('.settings-nav-item').filter({ hasText: 'Tax Rates' })
