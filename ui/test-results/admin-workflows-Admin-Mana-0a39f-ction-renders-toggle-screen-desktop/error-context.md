...
- Name: admin-workflows.spec.ts >> Admin Management Screens >> Features section renders toggle screen
- Location: e2e\admin-workflows.spec.ts:150:3
...
Locator: locator('.settings-nav-item').filter({ hasText: 'Features' })
Expected: visible
Timeout: 3000ms
...
Call log:
  ...
  - waiting for locator('.settings-nav-item').filter({ hasText: 'Features' })
