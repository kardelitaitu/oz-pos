...
- Name: admin-workflows.spec.ts >> Admin Management Screens >> Data Management section renders with tabs
- Location: e2e\admin-workflows.spec.ts:162:3
...
Locator: locator('.settings-nav-item').filter({ hasText: 'Data' })
Expected: visible
Timeout: 3000ms
...
Call log:
  ...
  - waiting for locator('.settings-nav-item').filter({ hasText: 'Data' })
