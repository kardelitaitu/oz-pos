...
- Name: admin-workflows.spec.ts >> Admin Management Screens >> Promotions section renders with table
- Location: e2e\admin-workflows.spec.ts:113:3
...
Locator: locator('.settings-nav-item').filter({ hasText: 'Promotions' })
Expected: visible
Timeout: 3000ms
...
Call log:
  ...
  - waiting for locator('.settings-nav-item').filter({ hasText: 'Promotions' })
