...
- Name: admin-workflows.spec.ts >> Admin Management Screens >> About section renders version info
- Location: e2e\admin-workflows.spec.ts:174:3
...
Locator: locator('.settings-nav-item').filter({ hasText: 'About' })
Expected: visible
Timeout: 3000ms
...
Call log:
  ...
  - waiting for locator('.settings-nav-item').filter({ hasText: 'About' })
