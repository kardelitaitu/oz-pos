...
- Name: admin-workflows.spec.ts >> Admin Management Screens >> Staff section renders with table
- Location: e2e\admin-workflows.spec.ts:48:3
...
Locator: locator('.settings-nav-item').filter({ hasText: 'Staff' })
Expected: visible
Timeout: 3000ms
...
Call log:
  ...
  - waiting for locator('.settings-nav-item').filter({ hasText: 'Staff' })
