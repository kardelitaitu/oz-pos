...
- Name: admin-workflows.spec.ts >> Admin Management Screens >> Offline Queue section renders header
- Location: e2e\admin-workflows.spec.ts:100:3
...
Locator: locator('.settings-nav-item').filter({ hasText: 'Offline Queue' })
Expected: visible
Timeout: 3000ms
...
Call log:
  ...
  - waiting for locator('.settings-nav-item').filter({ hasText: 'Offline Queue' })
