...
- Name: admin-workflows.spec.ts >> Admin Management Screens >> Terminals section renders with table
- Location: e2e\admin-workflows.spec.ts:61:3
...
Locator: locator('.settings-nav-item').filter({ hasText: 'Terminals' })
Expected: visible
Timeout: 3000ms
...
Call log:
  ...
  - waiting for locator('.settings-nav-item').filter({ hasText: 'Terminals' })
