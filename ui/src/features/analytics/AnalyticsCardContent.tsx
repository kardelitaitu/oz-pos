...
function RankedList({ rows, ariaLabel }: { rows: RankRow[]; ariaLabel: string }) {
  ...
  return (
    <ul className="analytics-rank-list" aria-label={ariaLabel}>
      {rows.map((r, i) => (
        <li key={r.name} className="analytics-rank-row">
...
function RevenueCard({ g, title }: { g: Granularity; title: string }) {
  const { l10n } = useLocalization();
  const { fmt, short } = useMoney();
  const data = useMemo(() => series('revenue', g, 18_500_000, 1.35), [g]);
...
function AovCard({ g, title }: { g: Granularity; title: string }) {
  const { l10n } = useLocalization();
  const { fmt } = useMoney();
  const data = useMemo(() => series('aov', g, 187_000, 0.55), [g]);
...
function StaffCard({ g, title }: { g: Granularity; title: string }) {
  ...
  const rows = useMemo(() => {
    ...
    return names.map((name, i) => ({ name, value: values[i]!, display: short(values[i]!) }));
  }, [g, names, short]);
  return (
    <Visual>
...
function CustomersCard({ g, title }: { g: Granularity; title: string }) {
  const { l10n } = useLocalization();
...
function PaymentsCard({ title }: { title: string }) {
  const { l10n } = useLocalization();
  const segs = [
  ...
  const pcts = shares('payments', segs.length);
  const option = useMemo(() => ({
...
function DiscountsCard({ g, title }: { g: Granularity; title: string }) {
  const { l10n } = useLocalization();
  ...
  const rows = useMemo(() => {
    ...
    return names.map((name, i) => ({ name, value: values[i]!, display: `${values[i]} ${l10n.getString('analytics-card-discounts-redeemed')}` }));
  }, [g, names, l10n]);
  return (
    <Visual>
      <div className="analytics-kpi-row">
        <Kpi value="8.4%" label={l10n.getString('analytics-card-discounts-share')} />
...
function RefundsCard({ g }: { g: Granularity }) {
  const { l10n } = useLocalization();
  const { fmt } = useMoney();
  const data = useMemo(() => series('refunds', g, 3_200_000, 1.1), [g]);
  const count = Math.round(seeded(`refunds-count:${g}`)() * 28) + 6;
...
function CategoryCard({ title }: { title: string }) {
  const names = ['CPU', 'GPU', 'RAM', 'Storage', 'Motherboard'];
  const pcts = shares('category', names.length);
  const option = useMemo(() => ({
    tooltip: { trigger: 'item' as const },
    series: [{
      type: 'pie' as const, radius: ['58%', '82%'], center: ['50%', '50%'],
...
function BasketCard({ g, title }: { g: Granularity; title: string }) {
  const { l10n } = useLocalization();
  const data = useMemo(() => {
...
function InventoryCard({ g, title }: { g: Granularity; title: string }) {
  const { l10n } = useLocalization();
  const data = useMemo(() => series('inventory', g, 42, 0.8), [g]);
  const option = useMemo(() => ({
...
function LowStockCard({ g, title }: { g: Granularity; title: string }) {
  const { l10n } = useLocalization();
  const rows = useMemo(() => {
    const rnd = seeded(`low-stock:${g}`);
    const items = [
      { name: 'RAM D4 16GB', base: 4 },
      { name: 'Thermal Paste MX-6', base: 7 },
      { name: 'PSU RM850x', base: 11 },
      { name: 'SSD P3 Plus', base: 14 },
    ];
    return items.map((it) => ({ ...it, stock: Math.max(1, it.base + Math.round(rnd() * 4) - 2) }));
  }, [g]);
  return (
    <Visual>
...
function TablesCard({ g, title }: { g: Granularity; title: string }) {
  const { l10n } = useLocalization();
  const data = useMemo(() => series('tables', g, 46, 0.9), [g]);
...
function OccupancyCard({ g, title }: { g: Granularity; title: string }) {
  const { l10n } = useLocalization();
...
function WaitstaffCard({ g, title }: { g: Granularity; title: string }) {
  const { l10n } = useLocalization();
  const names = ['Bima', 'Lina', 'Dedi', 'Yanti'];
  const rows = useMemo(() => {
    const rnd = seeded(`waitstaff:${g}`);
    const values = names.map(() => Math.round(85 * (0.5 + rnd() * 1.1)));
    return names.map((name, i) => ({ name, value: values[i]!, display: `${values[i]} ${l10n.getString('analytics-card-waitstaff-covers')}` }));
  }, [g, names, l10n]);
  return (
    <Visual>
      <RankedList rows={rows} ariaLabel={title} />
...
function VoidsCard({ g, title }: { g: Granularity; title: string }) {
  const { l10n } = useLocalization();
  const names = ['Caffè Latte', 'Iced Coffee', 'Avocado Toast', 'Smoothie'];
  const rows = useMemo(() => {
    const rnd = seeded(`voids:${g}`);
    const values = names.map(() => Math.round(9 * (0.5 + rnd() * 1.3)));
    return names.map((name, i) => ({ name, value: values[i]!, display: `${values[i]}×` }));
  }, [g, names]);
  return (
    <Visual>
      <div className="analytics-kpi-row">
...
export function AnalyticsCardContent({ cardKey, granularity, workspaceView, title }: AnalyticsCardContentProps) {
  switch (cardKey) {
    case 'revenue': return <RevenueCard g={granularity} title={title} />;
    case 'aov': return <AovCard g={granularity} title={title} />;
    case 'staff': return <StaffCard g={granularity} title={title} />;
    case 'customers': return <CustomersCard g={granularity} title={title} />;
    case 'payments': return <PaymentsCard title={title} />;
    case 'discounts': return <DiscountsCard g={granularity} title={title} />;
    case 'refunds': return <RefundsCard g={granularity} />;
    case 'top-items': return <TopItemsCard g={granularity} title={title} workspace={workspaceView} />;
    case 'category': return <CategoryCard title={title} />;
    case 'basket': return <BasketCard g={granularity} title={title} />;
    case 'inventory': return <InventoryCard g={granularity} title={title} />;
    case 'low-stock': return <LowStockCard g={granularity} title={title} />;
    case 'tables': return <TablesCard g={granularity} title={title} />;
    case 'occupancy': return <OccupancyCard g={granularity} title={title} />;
    case 'waitstaff': return <WaitstaffCard g={granularity} title={title} />;
    case 'voids': return <VoidsCard g={granularity} title={title} />;
    default: return null;
  }
}
