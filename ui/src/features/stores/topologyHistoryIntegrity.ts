 ...
 * signal — the guards surface it as a `[topology]` console diagnostic
 * instead of absorbing it silently, so a future creation-path regression
...
export function validWiresForNodes<TNode extends { id: string }, TWire extends { id: string; fromNodeId: string; toNodeId: string }>(
  ...
  if (dropped.length > 0) {
    console.warn(
      `[topology] ${boundary}-time guard dropped ${dropped.length} dangling wire(s) ` +
