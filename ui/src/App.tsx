...
registerNavItem({ route: 'inventory', label: 'Inventory', requiredRole: 'manager', icon: icon('M16 4h2a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2h2', <rect x="8" y="2" width="8" height="4" rx="1" ry="1" />, <line x1="8" y1="12" x2="16" y2="12" />, <line x1="8" y1="16" x2="14" y2="16" />) });

registerPage({ route: 'inventory-adjustment', component: InventoryAdjustmentScreen, label: 'Stock Adjust', requiredRole: 'manager' });
registerNavItem({ route: 'inventory-adjustment', label: 'Stock Adjust', requiredRole: 'manager', icon: icon('M12 5v14M5 12h14', <line x1="12" y1="5" x2="12" y2="19" />, <line x1="5" y1="12" x2="19" y2="12" />) });

registerPage({ route: 'sales-history', component: SalesHistoryScreen, label: 'Sales History', feature: 'simple-retail' });
registerNavItem({ route: 'sales-history', label: 'Sales History', feature: 'simple-retail', icon: icon('M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z', <polyline points="14 2 14 8 20 8" />, <line x1="16" y1="13" x2="8" y2="13" />, <line x1="16" y1="17" x2="8" y2="17" />) });

registerPage({ route: 'sales-dashboard', component: SalesDashboardScreen, label: 'Dashboard', feature: 'simple-retail' });
registerNavItem({ route: 'sales-dashboard', label: 'Dashboard', feature: 'simple-retail', icon: icon('M3 3h7v7H3zM14 3h7v7h-7zM14 14h7v7h-7zM3 14h7v7H3z') });

registerPage({ route: 'eod-report', component: EodReportScreen, label: 'EOD Report', requiredRole: 'manager' });
registerNavItem({ route: 'eod-report', label: 'EOD Report', requiredRole: 'manager', icon: icon('M21.21 15.89A10 10 0 1 1 8 2.83', <path d="M22 12A10 10 0 0 0 12 2v10z" />) });

registerPage({ route: 'orders', component: VoidOrdersScreen, label: 'Orders', feature: 'simple-retail', requiredRole: 'manager' });
registerNavItem({ route: 'orders', label: 'Orders', feature: 'simple-retail', requiredRole: 'manager', icon: icon('M9 5H7a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2V7a2 2 0 0 0-2-2h-2', <rect x="9" y="3" width="6" height="4" rx="1" />, <path d="M9 14l2 2 4-4" />) });

registerPage({ route: 'tax-config', component: TaxConfigurationScreen, label: 'Tax Rates', feature: 'tax-engine', requiredRole: 'manager' });
registerNavItem({ route: 'tax-config', label: 'Tax Rates', feature: 'tax-engine', requiredRole: 'manager', icon: icon('M12 1v22M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6', <line x1="12" y1="1" x2="12" y2="23" />) });

registerPage({ route: 'exchange-rates', component: ExchangeRateScreen, label: 'Exchange Rates', requiredRole: 'manager' });
registerNavItem({ route: 'exchange-rates', label: 'Exchange Rates', requiredRole: 'manager', icon: icon('M12 1v22M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6', <line x1="12" y1="1" x2="12" y2="23" />) });
...
registerNavItem({ route: 'categories', label: 'Categories', feature: 'categories-enabled', requiredRole: 'manager', icon: icon('M4 6h16M4 12h16M4 18h10') });
...
registerNavItem({ route: 'data-management', label: 'Data', requiredRole: 'owner', icon: icon('M12 5c-5 0-9 1.34-9 3s4 3 9 3 9-1.34 9-3-4-3-9-3z', <ellipse cx="12" cy="5" rx="9" ry="3" />, <path d="M21 12c0 1.66-4 3-9 3s-9-1.34-9-3" />, <path d="M3 5v14c0 1.66 4 3 9 3s9-1.34 9-3V5" />) });

registerPage({ route: 'audit-log', component: AuditLogScreen, label: 'Audit Log', requiredRole: 'manager' });
registerNavItem({ route: 'audit-log', label: 'Audit Log', requiredRole: 'manager', icon: icon('M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z', <polyline points="14 2 14 8 20 8" />, <line x1="16" y1="13" x2="8" y2="13" />, <line x1="16" y1="17" x2="8" y2="17" />) });

registerPage({ route: 'offline-queue', component: OfflineQueueScreen, label: 'Offline Queue', requiredRole: 'manager' });
registerNavItem({ route: 'offline-queue', label: 'Offline Queue', requiredRole: 'manager', icon: icon('M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z') });
