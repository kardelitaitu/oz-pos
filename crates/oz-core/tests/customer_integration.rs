...
use oz_core::{Store, migrations, Customer};
...
fn new_customer_defaults_to_zero_loyalty_and_spending() {
    ...
    assert_eq!(c.loyalty_points, 0, "new customers start with 0 loyalty points");
    assert_eq!(c.total_spent_minor, 0, "new customers start with 0 spending");
...
fn loyalty_points_persist_after_raw_sql_update() {
    ...
    assert_eq!(loaded.loyalty_points, 50, "loyalty points should accumulate");
}
...
fn loyalty_points_can_increase_and_decrease() {
    let conn = setup();
    let s = store(&conn);
    let c = s.create_customer("Point Tracker", None, None, None).unwrap();
...
fn spending_persists_across_multiple_sessions() {
    ...
    let loaded = s.get_customer(&c.id).unwrap().unwrap();
    assert_eq!(loaded.total_spent_minor, 14799, "3500 + 10000 + 1299 = 14799");
    assert_eq!(loaded.loyalty_points, 0, "spending updates don't affect loyalty by default");
}
...
fn update_customer_preserves_loyalty_and_spending() {
    ...
    let updated = s.update_customer("crm-1", "Alice Smith", Some("alice.smith@example.com"), Some("+1-555-9999"), Some("Updated contact")).unwrap();
    assert_eq!(updated.name, "Alice Smith");
    assert_eq!(updated.email.as_deref(), Some("alice.smith@example.com"));
    assert_eq!(updated.loyalty_points, 150, "loyalty points preserved after contact update");
    assert_eq!(updated.total_spent_minor, 15000, "spending preserved after contact update");
}
...
fn large_loyalty_points_value() {
    ...
    let loaded = s.get_customer(&c.id).unwrap().unwrap();
    assert_eq!(loaded.loyalty_points, 999_999_999, "large loyalty values should roundtrip");
}
...
fn large_total_spent_value() {
    ...
    let loaded = s.get_customer(&c.id).unwrap().unwrap();
    assert_eq!(loaded.total_spent_minor, 2_147_483_647, "large spending values should roundtrip");
}
...
fn created_at_is_set_on_creation() {
    let conn = setup();
    let s = store(&conn);
    let c = s.create_customer("Timestamp Test", None, None, None).unwrap();
...
fn updated_at_increases_on_update() {
    let conn = setup();
    let s = store(&conn);
    let c = s.create_customer("Update Tracker", None, None, None).unwrap();

    std::thread::sleep(std::time::Duration::from_millis(10));

    let updated = s.update_customer(&c.id, "Update Tracker", None, None, None).unwrap();
