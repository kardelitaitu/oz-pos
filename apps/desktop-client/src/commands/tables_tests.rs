...
#[tokio::test]
async fn staff_denied_update_table() {
    let conn = oz_core::migrations::fresh_db();
    seed_owner(&conn);
    seed_staff(&conn);
    let state = scoped_state(conn, "owner-tok", "user-owner", "role-owner", "s1");
    ...
    let created = {
        ...
        store.create_table(&make_table("T1")).unwrap()
    };
    ...
    table.name = "hacked".into();
    let result = update_table_scoped("staff-tok".into(), table, app.state()).await;
    assert!(matches!(result, Err(AppError::PermissionDenied(_))));
}

#[tokio::test]
async fn staff_denied_delete_table() {
    let conn = oz_core::migrations::fresh_db();
    seed_owner(&conn);
    seed_staff(&conn);
    let state = scoped_state(conn, "owner-tok", "user-owner", "role-owner", "s1");

    // Create a table via the legacy owner path.
    let created = {
        let db = state.db.lock().await;
        let store = Store::new(&db);
        store.create_table(&make_table("T1")).unwrap().id.clone()
    };
    ...
    let result = delete_table_scoped("staff-tok".into(), created, app.state()).await;
    assert!(matches!(result, Err(AppError::PermissionDenied(_))));
}
