...
pub(crate) fn shared_topology_semantics() -> &'static Value {
...
pub(crate) fn shared_port_set_contains(path: &str, port_id: Option<&str>) -> bool {
...
pub(crate) fn is_warehouse_primary_input_port(port_id: Option<&str>) -> bool {
    shared_port_set_contains("/warehouse/primaryInputs", port_id)
}

pub(crate) fn is_warehouse_operational_input_port(port_id: Option<&str>) -> bool {
    shared_port_set_contains("/warehouse/operationalInputs", port_id)
}

pub(crate) fn shared_semantic_pairing_contains(
...
pub(crate) fn value_string<'a>(value: &'a Value, key: &str) -> Option<&'a str> {
...
pub(crate) fn has_semantic_fields(nodes: &[Value], wires: &[Value]) -> bool {
    nodes.iter().any(|node| {
...
pub(crate) fn semantic_branch_profile_id<'a>(
    nodes: &'a [Value],
    wires: &[Value],
) -> Option<&'a str> {
...
pub(crate) fn semantic_type_key(node: &Value) -> &str {
    node.get("metadata")
        .and_then(|metadata| value_string(metadata, "typeKey"))
        .unwrap_or("store-pos")
}

pub(crate) fn semantic_node_type(node: &Value) -> Option<&str> {
    value_string(node, "type")
}
...
pub(crate) fn ambiguous_legacy_wire(nodes: &[Value], wire: &Value) -> bool {
...
pub(crate) fn find_directed_cycle_node(nodes: &[Value], wires: &[Value]) -> Option<String> {
...
pub(crate) fn semantic_wire_matches_contract(
    wire: &Value,
    from_node: &Value,
    to_node: &Value,
) -> bool {
