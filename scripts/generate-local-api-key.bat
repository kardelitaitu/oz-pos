...
powershell -NoProfile -Command "$ErrorActionPreference='Stop'; try { $r = (Invoke-RestMethod -Uri 'http://localhost:3099/api/v1/tokens' -Method Post -Body '{\"label\":\"local-pos-client\",\"expiry_hours\":87600}' -ContentType 'application/json').token; Write-Host '============================================================================' -ForegroundColor Cyan; Write-Host 'Your Local API Key (valid for 10 years):' -ForegroundColor Green; Write-Host ''; Write-Host $r -ForegroundColor Yellow; Write-Host ''; Write-Host 'Copy the token string above and paste it into Settings -> Cloud Sync -> API Key' -ForegroundColor White; Write-Host '============================================================================' -ForegroundColor Cyan } catch { Write-Host 'ERROR: Failed to generate token. Please ensure scripts\start-local-sync.bat is running.' -ForegroundColor Red; exit 1 }"

if errorlevel 1 (
    echo.
    echo Make sure Docker Desktop is running and scripts\start-local-sync.bat has completed.
)
