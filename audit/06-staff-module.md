...
> **Residual update (2026-08-06):** the session-mint half of the pre-session picker residual is **CLOSED**. `Store::verify_instance_access` (the gate `create_session` calls in both clients) now resolves the caller from `users` and fails closed for unknown users, inactive users, and claimed `role_id` values that differ from the user's actual database role — a caller who knows an owner's user id can no longer mint a session as that owner (privilege escalation) or in an unassigned store's active instance (cross-store session minting). The remaining picker surface is the pre-session `list_workspaces`/`list_workspace_screens` reads, which still trust the claimed role for listing (workspace-name disclosure only, no data access); the architectural fix remains a server-issued post-login picker credential or a fully server-resolved picker.
