...
> **Status:** REMEDIATED IN PART · security boundary, atomicity, bounds, destructive-UX, rounding-policy, and delete-dependency-policy fixes implemented; module-migration and English-fallback items remain  
...
**TAX-09 (P3)** — New user-facing strings added to both `tax.ftl` and `tax.id.ftl` (bundle parity preserved). Existing `|| '…'` fallbacks in the component were left as defensive fallbacks; removing them is a P3 cleanup tracked below.
...
1. **TAX-05 — Rounding policy:** ✅ DONE (Phase 1). `RoundingMode` (Truncate/HalfUp) implemented in the tax module, threaded through all compute paths, golden-tested. Residual: the mode is not yet selectable per-store/jurisdiction via settings (currently a hardcoded `HalfUp` default at the command layer) — wiring it to `WorkspaceStorePosSettings` is a follow-up.
