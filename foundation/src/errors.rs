...
mod tests {
    ...
    fn not_found_error_display() {
        let err = NotFoundError { entity: "product", id: "SKU-999".into() };
    ...
    fn conflict_error_display() {
        let err = ConflictError { entity: "product", field: "sku" };
        assert_eq!(err.to_string(), "product conflict on sku");
    }

    #[test]
    fn conflict_error_debug() {
        let err = ConflictError { entity: "category", field: "name" };
        assert!(!format!("{err:?}").is_empty());
    }

    #[test]
    fn validation_error_display() {
        let err = ValidationError { field: "price", message: "must be positive".into() };
        assert_eq!(err.to_string(), "validation failed on price: must be positive");
    }

    #[test]
    fn validation_error_debug() {
        let err = ValidationError { field: "email", message: "invalid format".into() };
        assert!(!format!("{err:?}").is_empty());
    }

    #[test]
    fn not_found_error_fields() {
        let err = NotFoundError { entity: "user", id: "user-42".into() };
    ...
    fn conflict_error_fields() {
        let err = ConflictError { entity: "role", field: "id" };
    ...
    fn validation_error_fields() {
        let err = ValidationError { field: "name", message: "too short".into() };
    ...
    fn not_found_error_implements_std_error() {
        let err = NotFoundError { entity: "x", id: "y".into() };
        let _: &dyn std::error::Error = &err;
    }

    #[test]
    fn conflict_error_implements_std_error() {
        let err = ConflictError { entity: "x", field: "y" };
        let _: &dyn std::error::Error = &err;
    }

    #[test]
    fn validation_error_implements_std_error() {
        let err = ValidationError { field: "x", message: "y".into() };
        let _: &dyn std::error::Error = &err;
    }
}
