...
> **What CI actually runs.** `.github/workflows/dev-ci.yml` ("Dev CI") is the **only live workflow** — every other file in that directory is `.bak` and GitHub never executes it (`ci.yml.bak`, `nightly.yml.bak`, `release.yml.bak`, `e2e-pr.yml.bak`, `security.yml.bak`, `android.yml.bak`, `ios.yml.bak`, `deploy.yml.bak`, `website.yml.bak`, `docker-*.yml.bak`), per `23c96330`. Its jobs are `changes` (the path router), `website`, `cargo-check`, `cargo-nextest`, `ui-test`, `i18n`, `ci-docs-drift`, `static-gates`, and `northflank-deploy`. `northflank-deploy` `needs` every other live job **except `ci-docs-drift`**, which is gated on `needs.changes.outputs.docs` — a skipped dependency skips the job, so depending on it would block deploys on docs changes. So **E2E, a11y, release, security and nightly suites are NOT enforced in CI** — a green Dev CI run is not proof those passed. `scripts/check.sh` is the local full-matrix equivalent — **but note it currently fails at `panic-inventory`** (18 findings, 6 of them the scanner misreading the repo's own `/* */` audit stamps; tracked as R36-12), so a red `check.sh` is not automatically your change's fault: run it, then check whether the failure is that one. `cargo nextest run --workspace --all-features` in CI carries **no `--exclude`**, so CI tests the app crates that `check.sh` skips.
>
