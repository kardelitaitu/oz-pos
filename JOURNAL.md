...
**Follow-ups:** the remote PG query filters on `synced_at` — if the remote never stamps it, an anchored pull returns nothing new; consider a `created_at`-based cursor when a real multi-terminal PG deployment appears.
