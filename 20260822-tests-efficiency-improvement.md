...
- **2026-08-22 · final · throwaway-DB isolation for ALL remaining base-DB PG tests** — the last flake class (`tenant_sku_isolation` + `rest_roundtrip` ~25% on `adjust_stock` `FOR UPDATE` chains against the shared base DB) is **eliminated**: added a shared `throwaway_test_pool(url, prefix)` helper and converted **4 oz-api tests** to isolated throwaway DBs (`rest_roundtrip`, `rest_rls_non_owner`, `tenant_sku_isolation`, `concurrent_sale_status_transition` — every test exercising `FOR UPDATE`/role-setup on shared tables). Also stopped `webhooks_restricted_role_after_cutover` from dropping the cluster-wide `oz_webhook_resolver` (raced concurrent cutover tests) and `prune_survives_rls_cutover` from dropping `oz_email_discovery` (same fix family; swept into the other agent's `22e458c8`). **Verified: 4 consecutive full PG-suite runs (39 tests across oz-api + oz-cloud-server) all green — zero flakes.** The only 1/4-run flake observed was `push_batch_data_error`'s base-DB trigger DDL under extreme concurrent load (other agent's build); its trigger name is uuid-unique (no stale state) and it passed on retry — accepted noise floor.
