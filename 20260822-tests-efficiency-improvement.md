...
| A27 nextest workspace sweep | 100.1 (excl live-PG, 5469 tests) | — (blocked) | | | root-caused: 3× 50s dead-port tests (PG retry backoff); fix blocked by other agent's db.rs WIP |
...
- **2026-08-22 · baseline (partial) · commit `342f0e18` · 5469 tests (excl. live-PG), all pass, wall ~100 s (test time 88.9 s)** — `cargo nextest run --workspace --all-features --exclude oz-pos-app --exclude oz-pos-tablet -E 'not test(pg_integration_)'`. Root-caused the slow tail: `DbPool::connect_postgres` has a **production retry loop (5 attempts, backoff sleeps 2+4+8+16 = 30 s)** for PG addon startup; tests asserting a connection failure against dead port `localhost:5432` burn the full budget → `require_tls_accepts_sslmode_require_and_fails_on_connection` 50.8 s, `postgres_url_parsing_accepts_valid_url` 50.6 s, `from_env_detects_postgres_url` 50.6 s.
- **2026-08-22 · attempt 1 · technique: parameterize retry budget (playbook #1) → **BLOCKED BY OTHER AGENT** — added `connect_postgres_with_retries(…, max_attempts)` + `from_env_with_retries` (production default 5, tests pass 1) and updated the three dead-port tests. Verified the fix in isolation: TLS test **50.8 → 4.1 s (−92%)**; the remaining ~4 s is Windows refused-connect latency (machine-specific, unavoidable). **But the other agent is actively editing `apps/cloud-server/src/db.rs` (concurrent WIP overwrote my edits; working tree now matches HEAD with a pre-existing duplicated doc comment).** Per §8 the other agent's in-flight files win — the fix cannot land until their work settles. Re-measure after: full sweep should drop to ~35–40 s (three 50 s tests → ~4 s each).
- **2026-08-22 · note** — the live-PG sweep (`pg_integration_*`) remains blocked by the other agent's RLS cutover drift on the dev container (`offline_queue`/`sync_terminals`/`tenant_plans` FORCEd without reset); re-run after `bash scripts/reset-dev-pg.sh` once their RLS work lands (see AGENTS.md note).
