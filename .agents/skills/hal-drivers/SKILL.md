...
- **The registry id is the contract.** Commands look up fixed strings (`printer("default")`, `printer("kitchen")`, `scale("default")`), while `discover()` mints hardware-derived ids like `printer:vendor:model`. Calling `discover()` therefore does not make a lookup resolve. Any device an operator configures must be registered under the id the command asks for — that binding is what `apply_config` exists for, and it is the reason `discover()` alone left every hardware command returning `None`.
- `discover()` is the auto-probe phase (USB/Bluetooth/serial); failure of one driver does not abort discovery. It is not the startup registration path, and no app calls it today: it binds ids nothing looks up and opens serial/Bluetooth ports nobody named.
- A device the operator did not name must not appear because it was found. Card terminals are deliberately absent from `discover()` for this reason — see `discover_never_registers_a_card_terminal`.
...
- [ ] Make it reachable. If the operator configures it, add a `HardwareConfig` entry in `bootstrap.rs` and map the profile field in `platform_startup::hardware::config_from_profile`, registering under the **exact id the command looks up**. Only add to `discover()` if the device should be bound without being configured — and note that discovery's ids are hardware-derived, so a fixed-string lookup will not find them.
